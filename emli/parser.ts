...
// Logger "EMLI compiler | PARSER"
const logger = new Logger.Builder().setName('EMLI compiler | PARSER').build();

// Read grammar.ohm
logger.info('Reading EMLI grammar...');
const grammar = ohm.grammar(utils.read('grammar.ohm'));
...
const semantics = grammar.createSemantics().addOperation('toIntermediate', {
  ...
  string: (_q1, text, _q2) => new CustomHTMLIntermediary(text.sourceString),
